---
name: gdd
description: "Create, update, review, or query the Game Design Document (GDD) at documentation/game-design-document.md. Use this skill when the user wants to work on their GDD."
---

# Game Design Document (GDD) Management Skill

You are managing the Game Design Document for this project. The GDD lives at `documentation/game-design-document.md` as a single comprehensive Markdown file.
...
### `/gdd` (no arguments)
- If `documentation/game-design-document.md` exists, read it and display a brief status summary: which sections exist, which are stubs/empty, and when it was last modified (via git log).
- If `documentation/game-design-document.md` does not exist, ask the user if they'd like to create the initial GDD template.

### `/gdd init`
- Create the initial GDD template at `documentation/game-design-document.md` with all standard sections.
...
### `/gdd roadmap`
- Read and display the current roadmap from `documentation/roadmap.md`.
- If the roadmap doesn't exist, create it using the Mermaid gantt template below.
- When updating, preserve the Mermaid gantt format and keep task IDs consistent.

### `/gdd roadmap update`
- Read the current roadmap and ask the user what changes to make (add tasks, update dates, mark completion, add sections).
- Completed tasks should use the `done` tag (e.g., `:done, design1, 2026-02-14, 1d`).
- Active tasks should use the `active` tag.
- Keep the roadmap in sync with GDD Section 11 (Production) — update both when milestones change.

## Roadmap Template

The roadmap lives at `documentation/roadmap.md` and uses a Mermaid gantt chart. When creating or updating:
...
```mermaid
gantt
    title [Game Name] - Development Roadmap
...
**Roadmap rules:**
- Use `done` for completed tasks, `active` for in-progress tasks, no tag for pending tasks.
- Task IDs should be short and namespaced by section (e.g., `design1`, `core2`, `sys3`).
- Keep sections aligned with GDD Section 11 development phases.
- When a milestone is completed in the GDD, mark corresponding roadmap tasks as `done`.
...
9. **Always use Mermaid diagrams** for flowcharts, loops, state machines, hierarchies, and system relationships unless the user explicitly requests otherwise. Use `flowchart`, `stateDiagram-v2`, `sequenceDiagram`, or `graph` blocks — never ASCII art.
10. **Keep the roadmap in sync** with GDD Section 11 (Production). When updating milestones or phases in either file, update the other to match. The roadmap lives at `documentation/roadmap.md`.
